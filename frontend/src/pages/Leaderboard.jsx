    ...
    <div className="bg-surface-container-lowest text-on-surface font-body antialiased min-h-screen flex flex-col">
      ...
      <div className="flex-1 flex flex-col md:flex-row max-w-[1600px] mx-auto w-full" style={{ marginTop: '100px' }}>
        ...
        <main className="flex-1 p-6 pt-6 md:p-12 lg:pl-16 lg:pr-12 xl:pr-16 flex flex-col gap-12 overflow-y-auto">
          ...
          {/* Podium */}
          {top3.length >= 3 && (
            <section className="flex flex-col md:flex-row justify-center items-end gap-6 h-80 mt-4 mb-8">
              {/* Rank 2 (Left) */}
              <div className="bg-surface-variant/50 backdrop-blur-md rounded-xl p-6 flex flex-col items-center w-48 relative order-2 md:order-1 h-64 justify-end">
                <div className="absolute -top-12">
                  <div className="relative rounded-full p-[3px] bg-gradient-to-b from-[#C0C0C0] to-[#C0C0C0]/20 shadow-[0_0_15px_rgba(192,192,192,0.1)] overflow-hidden flex items-center justify-center">
                    <div className="rounded-full border-2 border-surface-container-low overflow-hidden w-20 h-20 bg-surface-container">
                      <Avatar config={top3[1]?.avatar_config} size={80} showBg={true} />
                    </div>
                  </div>
                </div>
                <span className="font-headline font-bold text-[#C0C0C0] text-sm tracking-widest mt-6">RANK 2</span>
                <h3 className="font-display font-bold text-on-surface mt-2 text-center text-lg truncate w-full">{top3[1]?.name}</h3>
                <div className="font-mono text-primary mt-1 text-sm">{top3[1]?.xp?.toLocaleString()} XP</div>
              </div>

              {/* Rank 1 (Center) */}
              <div className="bg-surface-variant/50 backdrop-blur-md rounded-xl p-6 flex flex-col items-center w-56 relative order-1 md:order-2 h-full justify-end border-t border-[#FFD700]/30 shadow-[0_-10px_30px_rgba(255,215,0,0.1)]">
                <div className="absolute -top-16">
                  <div className="relative rounded-full p-[4px] bg-gradient-to-b from-[#FFD700] to-[#FFD700]/20 shadow-[0_0_20px_rgba(255,215,0,0.15)] overflow-hidden flex items-center justify-center">
                    <div className="rounded-full border-4 border-surface-container-lowest overflow-hidden w-24 h-24 bg-surface-container">
                      <Avatar config={top3[0]?.avatar_config} size={96} showBg={true} />
                    </div>
                  </div>
                  <div className="absolute -bottom-2 -right-2 bg-[#FFD700] text-surface-container-lowest w-8 h-8 rounded-full flex items-center justify-center font-display font-black text-sm z-10 shadow-lg">1</div>
                </div>
                <span className="font-headline font-bold text-[#FFD700] text-sm tracking-widest mt-8">RANK 1</span>
                <h3 className="font-display font-bold text-on-surface mt-2 text-center text-xl truncate w-full">{top3[0]?.name}</h3>
                <div className="font-mono text-primary mt-2 text-base font-bold drop-shadow-[0_0_8px_rgba(0,229,255,0.4)]">{top3[0]?.xp?.toLocaleString()} XP</div>
              </div>

              {/* Rank 3 (Right) */}
              <div className="bg-surface-variant/50 backdrop-blur-md rounded-xl p-6 flex flex-col items-center w-48 relative order-3 h-56 justify-end">
                <div className="absolute -top-10">
                  <div className="relative rounded-full p-[2px] bg-gradient-to-b from-[#CD7F32] to-[#CD7F32]/20 shadow-[0_0_15px_rgba(205,127,50,0.1)] overflow-hidden flex items-center justify-center">
                    <div className="rounded-full border-2 border-surface-container-low overflow-hidden w-16 h-16 bg-surface-container">
                      <Avatar config={top3[2]?.avatar_config} size={64} showBg={true} />
                    </div>
                  </div>
                </div>
                <span className="font-headline font-bold text-[#CD7F32] text-sm tracking-widest mt-4">RANK 3</span>
                <h3 className="font-display font-bold text-on-surface mt-2 text-center text-lg truncate w-full">{top3[2]?.name}</h3>
                <div className="font-mono text-primary mt-1 text-sm">{top3[2]?.xp?.toLocaleString()} XP</div>
              </div>
            </section>
          )}
          ...
          <div className="bg-surface-container-low rounded-lg overflow-hidden flex flex-col">
            <div className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-outline-variant/15 font-headline text-xs text-on-surface-variant font-bold tracking-wider uppercase bg-surface-container">
            ...
            <div className="flex flex-col">
              {rest.map((entry, index) => {
                ...
                return (
                  <div key={entry.id} className={`grid grid-cols-12 gap-4 px-6 py-4 items-center transition-colors ${
                    isMe 
                    ...
                    {isMe && <div className="absolute top-0 left-0 w-1 h-full bg-primary"></div>}
                    ...
                    <div className="col-span-4 flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full overflow-hidden flex items-center justify-center bg-surface-container ${isMe ? 'border border-primary' : ''}`}>
                        <Avatar config={entry.avatar_config} size={32} showBg={false} />
                      </div>
                      <span className={`font-display text-sm truncate max-w-[120px] lg:max-w-[180px] ${isMe ? 'font-bold text-on-surface' : 'font-semibold text-on-surface'}`}>
                        {entry.name}
                      </span>
                      {isMe && <span className="bg-secondary-container text-on-secondary-container text-[10px] px-2 py-0.5 rounded-sm font-headline tracking-wider ml-2 hidden sm:inline-block">YOU</span>}
                    </div>
        ...
        {/* Right Sidebar */}
        <aside className="w-full md:w-[380px] lg:w-[420px] bg-surface-container-low p-6 md:p-8 flex flex-col gap-8 border-l border-outline-variant/15 overflow-y-auto">
          {/* Your Status Card */}
