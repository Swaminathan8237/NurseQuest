...
function seed() {
  ...
  db.prepare(`INSERT INTO users (id, email, password, name, role) VALUES (?, ?, ?, ?, ?)`).run(
    teacherId, 'teacher@nursequest.com', teacherPassword, 'Dr. Sarah Johnson', 'teacher'
  );
  ...
  studentNames.forEach((name, i) => {
    ...
    db.prepare(`INSERT INTO users (id, email, password, name, role, avatar_config, xp, level, streak) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
      id, `student${i + 1}@nursequest.com`, password, name, 'student', avatarConfigs[i], xps[i], Math.floor(xps[i] / 1000) + 1, Math.floor(Math.random() * 10)
    );
  });
  ...
  const mod2Id = uuidv4();
  const mod3Id = uuidv4();
  db.prepare(`INSERT INTO modules (id, title, description, icon, color, order_index, created_by, is_published) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(
    mod1Id, 'Fundamentals of Nursing', 'Core nursing concepts covering patient care, safety, and clinical procedures', 'health_and_safety', '#b76dff', 0, teacherId, 1
  );
  db.prepare(`INSERT INTO modules (id, title, description, icon, color, order_index, created_by, is_published) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(
    mod2Id, 'Anatomy & Physiology', 'Human body systems, structures, and physiological processes', 'biotech', '#71d7cd', 1, teacherId, 1
  );
  db.prepare(`INSERT INTO modules (id, title, description, icon, color, order_index, created_by, is_published) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(
    mod3Id, 'Pharmacology', 'Drug classifications, mechanisms, dosages, and interactions', 'medication', '#FF6B6B', 2, teacherId, 1
  );
  ...
  db.prepare(`INSERT INTO quizzes (id, title, description, category, difficulty, unit, module, time_per_question, created_by, is_published, module_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
    quiz1Id, 'Fundamentals of Patient Care', 'Test your knowledge on basic patient care procedures and protocols', 'Patient Care', 'easy', 1, 'Module 1', 30, teacherId, 1, mod1Id
  );

  const quiz2Id = uuidv4();
  db.prepare(`INSERT INTO quizzes (id, title, description, category, difficulty, unit, module, time_per_question, created_by, is_published, module_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
    quiz2Id, 'Anatomy & Physiology Challenge', 'Identify anatomical structures and understand body systems', 'Anatomy', 'medium', 2, 'Module 2', 25, teacherId, 1, mod2Id
  );

  const quiz3Id = uuidv4();
  db.prepare(`INSERT INTO quizzes (id, title, description, category, difficulty, unit, module, time_per_question, created_by, is_published, module_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
    quiz3Id, 'Pharmacology Essentials', 'Drug classifications, dosages, and interactions', 'Pharmacology', 'hard', 3, 'Module 3', 20, teacherId, 1, mod3Id
  );

  // Create 10 questions across different types
  const questions = [
    // Text MCQ
    {
  ...
  const insertQuestion = db.prepare(`INSERT INTO questions (id, quiz_id, type, question_text, media_url, options, correct_answer, explanation, points, order_index) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);

  questions.forEach(q => {
    insertQuestion.run(uuidv4(), q.quiz_id, q.type, q.question_text, q.media_url || null, q.options, q.correct_answer, q.explanation, 1000, q.order_index);
  });

  // Create quiz attempts for leaderboard data
  const insertAttempt = db.prepare(`INSERT INTO quiz_attempts (id, quiz_id, user_id, score, total_points, correct_count, total_questions, streak_max, time_taken) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`);

  studentIds.forEach((sid, i) => {
    insertAttempt.run(uuidv4(), quiz1Id, sid, 3000 + (i * 500), 4000, 3 + Math.min(i, 1), 4, 2 + Math.min(i, 2), 90 + i * 10);
    insertAttempt.run(uuidv4(), quiz2Id, sid, 2500 + (i * 400), 4000, 2 + Math.min(i, 2), 4, 1 + Math.min(i, 3), 100 + i * 5);
  });
  ...
  const insertAchievement = db.prepare(`INSERT INTO achievements (id, name, description, icon, requirement_type, requirement_value) VALUES (?, ?, ?, ?, ?, ?)`);
  const insertUserAchievement = db.prepare(`INSERT INTO user_achievements (id, user_id, achievement_id) VALUES (?, ?, ?)`);

  achievements.forEach(a => {
    const aid = uuidv4();
    insertAchievement.run(aid, a.name, a.description, a.icon, a.type, a.value);
    // Give first achievement to all students
    if (a.type === 'quizzes_completed' && a.value === 1) {
      studentIds.forEach(sid => {
        insertUserAchievement.run(uuidv4(), sid, aid);
      });
    }
  });
