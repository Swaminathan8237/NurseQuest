...
async function seedComprehensiveQuiz() {
  console.log('🌱 Starting comprehensive quiz seeding on PostgreSQL...');
  const sql = getDB();
  ...
  if (existingQuizzes.length > 0) {
    ...
    await sql`DELETE FROM questions WHERE quiz_id = ${existingId}`;
    await sql`DELETE FROM quiz_attempts WHERE quiz_id = ${existingId}`;
    await sql`DELETE FROM quizzes WHERE id = ${existingId}`;
  }
  ...
  console.log('🎉 Comprehensive quiz seeded successfully on PostgreSQL!');
  process.exit(0);
}
